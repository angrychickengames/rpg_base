...
public abstract class BaseGameService : MonoBehaviour
{
    ...
    /// <param name="onSuccess"></param>
    /// <param name="onError"></param>
    public void OpenLootBox(string lootBoxDataId, UnityAction<ItemResult> onSuccess = null, UnityAction<string> onError = null)
    {
        var player = Player.CurrentPlayer;
        var playerId = player.Id;
        var loginToken = player.LoginToken;
        HandleServiceCall();
        DoOpenLootBox(playerId, loginToken, lootBoxDataId, (finishResult) => HandleResult(finishResult, onSuccess, onError));
    }
    ...
    protected abstract void DoOpenLootBox(string playerId, string loginToken, string lootBoxDataId, UnityAction<ItemResult> onFinish);
}
